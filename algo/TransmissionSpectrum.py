...
class TransmissionSpectrum():
    def __init__(self,directory,init_wavelength = 776e-8,final_wavelength = 781e-8,):
        """"""
        self.single_scan_width = Scope.calculate_scan_width()
        for i in np.arange(init_wavelength, final_wavelength, self.single_scan_width):
            Laser.tlb_set_wavelength(i)
            time.sleep(WAIT_TIME)
            self.spectrum_sheet.append(Scope.save_trace())
        self.attach_spectrum_sheets(self.spectrum_sheet)
